...
type TMainField = string;
type TNestedField = `${string}.${string}`;
type TSelectableFields = "*" | string;

type TPopulateOption = `${TMainField}::${TSelectableFields}` | `${TNestedField}::${TSelectableFields}`;

type TProjectFieldValue = 1 | 0 | true | false | Record<string, any>;

// Type representing the project object structure
type TProject = Record<string, TProjectFieldValue>;
...
type TOptions = {
  ...
  postPopulateFilters: FilterQuery<any>,
  project: TProject
}
...
module.exports.paginate = (schema: Schema) => {
  schema.statics.paginate = async function(filters: FilterQuery<any>, options: TOptions, geoFilters?: TGeoFilters) {
    // defining pipeline
    const pipeline: PipelineStage[] = [];
    // adding location based search filters if given by user
    if (geoFilters) {
      const {longitude, latitude, fieldName = 'location', radius = 1} = geoFilters;
      pipeline.push({
        $geoNear: {
    ...
    let docsPipeline: PipelineStage[] = [...pipeline, ...(options.pipeline || [])];

    const removeUnwantedFields: string[] = [];
    ...
    if (options.populate) {
      ...
      if (options.postPopulateFilters) {
        docsPipeline.push({
          $match: options.postPopulateFilters,
        });
      }
    }
    ...
    return Promise.all([countPromise, docsPromise]).then(values => {
      ...
      const result: TPaginateResponse = {
        page,
        limit,
        results,
        totalPages,
